  ...
  const login = async () => {
    try {
      ...
      if (response.data?.success) {
        ...
        alertMsg(`Error: ${response.data?.message + response.error?.response?.data?.message}.`, 'Mensaje de Eugenia.', 'error');
      }
    }
    catch (e) {
      console.log(e);
        ...
        <div className="card-body">
          <>
            <div class="mb-3">
              ...
              <input type="email" class="form-control bg-dark text-white border border-success" laceholder="Enter email" required
                onChange={(event) => { setEmailUser(event.target.value) }} />
            </div>
            <div class="mb-3">
              <label>Password: </label>
              <input type="password" class="form-control bg-dark text-white border border-success" placeholder="Password" autocomplete="off" required
