...
import { getInvUserId, deleteInvId, createInvUser } from "../controllers/invitattions/invitations";
//
const Home = () => {
    //
    ...
    const [valueEntry, setValueEntry] = useState(dayjs(moment().format()));
    const [valueExp, setValueExp] = useState(dayjs(moment().format()));
    ...
    const fetchData = async () => {
        ...
        if (!localStorage.getItem("token")) {
            ...
            const dataIn = { id_user, id_inv: invId }
            const response = await getInvUserId(dataIn);
            const data = response.data.data;
            console.log('data: ', data);
            if (data.some(x => x.id_inv !== null)) {
                setListInvUser(data);
                alertMsg(`Excellent, all user invitations ${name_user}.`, 'Mensaje de Eugenia.', 'success');
            } else {
                setListInvUser([]);
                alertMsg(`Invitation: ${invId} does not exist in user: ${name_user}.`, 'Mensaje de Eugenia.', 'info');
            };
            setInvQuery(false);
        };
    };
        ...
        setInvInfo(
            {
                ...
                id_inv_status: '1',
                status_inv: 'Activa',
                entry_date_time_inv: moment(),
                expiration_date_inv: moment()
            }
        )
        console.log('invInfo New:', invInfo);
    };
    //
    const execNewInv = async () => {
        if (!localStorage.getItem("token")) {
            navigate("/login");
        } else {
            invInfo.entry_date_time_inv = valueEntry.format('YYYY-MM-DD HH:mm:00');
            invInfo.expiration_date_inv = valueExp.format('YYYY-MM-DD HH:mm:00');
            console.log('invInfo Val:', invInfo);
            const response = await createInvUser(invInfo);
            const data = response.data;
            console.log('data: ', data);
            alertMsg(`Excellent, new invitation created.`, 'Mensaje de Eugenia.', 'info');
            await fetchData();
        };
    };
    //
    const execRemoveInv = async () => {
        if (!localStorage.getItem("token")) {
            navigate("/login");
        } else {
            const response = await deleteInvId(invInfo.id_inv);
            const data = response.data;
            console.log('data: ', data);
            alertMsg(`Excellent, invitation ${invInfo.id_inv} removed.`, 'Mensaje de Eugenia.', 'info');
            await fetchData();
        };
    };
    //
    const searchInv = async () => {
        // 
        if (!localStorage.getItem("token")) {
            navigate("/login");
        } else {
            if (invId === '') { setInvId(null); }
            const dataIn = { id_user, id_inv: invId }
            console.log('dataIn search:', dataIn);
            const response = await getInvUserId(dataIn);
            const data = response.data.data;
            console.log('data: ', data);
            if (data.some(x => x.id_inv !== null)) {
                ...
                if (invId === null || invId === '') {
                    ...
                    alertMsg(`Invitation: ${invId} in user: ${name_user}.`, 'Mensaje de Eugenia.', 'success');
                }
            } else {
                alertMsg(`Invitation: ${invId} does not exist in user: ${name_user}.`, 'Mensaje de Eugenia.', 'info');
            };
    ...
    return (
        <LocalizationProvider dateAdapter={AdapterDayjs}>
            <div className="container-fluid">
                <div className="row">
                    ...
                    <div className="col-sm-3 mt-3">
                        <div className="d-flex">
                            <input id="searchInvLabel" className="form-control me-sm-2" type="search" placeholder="Search"
                                onChange={(event) => { setInvId(event.target.value) }} />
                            <button className="btn btn-secondary my-2 my-sm-0" data-placement="top" title="Click search"
                                onClick={() => searchInv(id_user)}>
                ...
                <div class="modal fader" id="modalDeleteInv" tabindex="-1" aria-labelledby="modalDeleteInv" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                ...
                                <h5 class="modal-title fs-5" id="modalDeleteInv">Warning!!! Invitation: {padLeft(invInfo.id_inv, 10)}</h5>
                            ...
                            <div class="modal-body">
                                <div class="card border-light bg-danger text-white">
                                    <div class="card-body">
                                        <div className="text-center">
                                            <h5>Are you sure to delete this invitation?</h5>
                                            <QRCode className="img-fluid rounded float-left img-thumbnail"
                                                size={256}
                                                style={{ height: "auto", maxWidth: "100%", width: "auto" }}
                                                value={`${UrlApi}/${invInfo.id_inv}`}
                ...
                <div class="modal fade" id="modalNewInv" tabindex="-1" aria-labelledby="modalNewInv" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                                    ...
                                    <div class="card-body">
                                        <div className="row mt-3">
                                            ...
                                            <div className="col-sm-10">
                                                ...
                                                <DateTimePicker
                                                    value={valueEntry}
                                                    onChange={(newValue) => setValueEntry(newValue)}
                                                />
                                                <h5 className="mt-2">Expiration: </h5>
                                                <DateTimePicker
                                                    value={valueExp}
                                                    onChange={(newValue) => setValueExp(newValue)}
                                                />
                                                <h5 className="mt-2">Status: {invInfo.status_inv}</h5>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="modal-footer">
                                <button id="btnOkNewInv" type="button" class="btn btn-success"
                                    onClick={() => execNewInv()}
                                    data-bs-dismiss="modal">
                                    Ok + <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-calendar4-event mb-1" viewBox="0 0 16 16">
                                        <path d="M3.5 0a.5.5 0 0 1 .5.5V1h8V.5a.5.5 0 0 1 1 0V1h1a2 2 0 0 1 2 2v11a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V3a2 2 0 0 1 2-2h1V.5a.5.5 0 0 1 .5-.5M2 2a1 1 0 0 0-1 1v1h14V3a1 1 0 0 0-1-1zm13 3H1v9a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1z" />
                                        <path d="M11 7.5a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5z" />
                                    </svg>
                                </button>
